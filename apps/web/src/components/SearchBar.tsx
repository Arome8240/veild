  ...
  return (
    <div className="relative">
      <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2">
        <svg className="h-4 w-4 shrink-0 text-zinc-500" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {query.length >= 2 && results.length > 0 && (
        <ul id="creator-search-results" role="listbox" aria-label="Creator search results" className="absolute left-0 right-0 top-full z-50 mt-1 rounded-xl border border-white/10 bg-zinc-900 py-1 shadow-xl">
          {results.map((c) => (
            <li key={c.username}>
              <button
                type="button"
